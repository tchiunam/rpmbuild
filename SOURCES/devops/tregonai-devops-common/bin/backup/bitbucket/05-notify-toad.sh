# summary: Notify all Toad instances to go online
# timeout: 2m
# entrypoint: /bin/bash
...
if [[ `hostname` =~ ^hkbitbucket ]]; then
    for host in ${prod_hosts[@]}; do
        ...
        curl -isS --insecure -X PUT -H "Content-Type: application/json" -d '{ "user" : "bitbucket", "token" : "<bitbucket_token>", "mode" : "online", "delay" : 600 }' https://${host}:5000/tofu/api/1.0/trading_service/misc/toadtsd/mode
    done
else
    for host in ${dev_hosts[@]}; do
        ...
        info Main::${LINENO} "Notifying ${host}"
        # Update with a valid token
        curl -isS --insecure -X PUT -H "Content-Type: application/json" -d '{ "user" : "bitbucket", "token" : "<bitbucket_token>", "mode" : "online", "delay" : 600 }' https://${host}:5000/tofu/api/1.0/trading_service/misc/toadtsd/mode > /dev/null 2>&1
    done
fi
set -e
